...
class Parser:
    ...
    def pipeline_of_preprocessing(self, f_name, bound=''):
        holder = {}
        inverted_index, num_of_rdf, similar_characteristics,vocabulary = self.inverted_index(f_name, bound)

        holder_values = self.similarity_measurer().get_similarities(inverted_index, num_of_rdf, self.K)

        for i in range(len(vocabulary)):
            holder[vocabulary[i]] = holder_values[i]
    ...
    def inverted_index(self, path, bound):
        ...
        return inverted_index, num_of_rdf, similar_characteristics, vocabulary
...
class PYKE(object):
    ...
    def go_through_entities(self, e, holder, omega):
        ...
        for target_index in range(len(e)):
            indexes_of_attractive, pms_of_contest, indexes_of_repulsive = list(holder.values())[target_index]
